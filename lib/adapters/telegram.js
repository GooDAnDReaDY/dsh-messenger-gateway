...
export class TelegramAdapter {
  ...
  wrapCallback(cq) {
    ...
    return {
      platform: 'telegram', chatId, userId: cq.from?.id, data: cq.data, callbackQueryId: cq.id,
  ...
  async handleMessage(msg) {
    ...
    const reply = async (payload) => this.sendReply(chatId, msg.message_id, payload)
  ...
  async sendReply(chatId, replyTo, payload) {
    const body = typeof payload === 'string' ? { text: payload } : (payload || {})
    const files = Array.isArray(body.files) ? body.files : []
    const text = String(body.text || '')
    const replyMarkup = body.replyMarkup
    for (const file of files) {
      try { await this.sendMedia(chatId, file) } catch (e) { this.logger?.warn?.(`send media: ${e.message}`) }
    }
    if (text) {
  ...
  async sendTo(chatId, payload) {
    return this.sendReply(chatId, undefined, payload)
  }
}
