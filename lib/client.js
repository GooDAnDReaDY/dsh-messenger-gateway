  ...
  factory: (require) => {
    ...
    function Section() {
        ...
        try {
          const body = {
            config: {
              ...
              agent: { ...cfg.agent, ...(patch.agent || {}) },
              telegram: { ...cfg.telegram, ...(patch.telegram || {}), textFormat: (patch.telegram || cfg.telegram)?.textFormat || cfg.telegram?.textFormat },
            },
          }
      ...
      return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 720 } },
        ...
        React.createElement('label', null, 'Agent provider (empty = default)'),
        React.createElement('input', { value: cfg.agent?.provider || '', onChange: (e) => setCfg({ ...cfg, agent: { ...cfg.agent, provider: e.target.value } }) }),
        React.createElement('label', null, 'Agent model (empty = default)'),
        React.createElement('input', { value: cfg.agent?.model || '', onChange: (e) => setCfg({ ...cfg, agent: { ...cfg.agent, model: e.target.value } }) }),
        React.createElement('label', null, 'Photo without caption'),
        React.createElement('select', { value: cfg.agent?.photoOnlyMode || 'prompt', onChange: (e) => setCfg({ ...cfg, agent: { ...cfg.agent, photoOnlyMode: e.target.value } }) },
          React.createElement('option', { value: 'prompt' }, 'Wait for question (recommended)'),
          React.createElement('option', { value: 'run' }, 'Reply immediately'),
        ),
        React.createElement('label', null, 'Text format'),
        React.createElement('select', { value: cfg.telegram?.textFormat || 'html', onChange: (e) => setCfg({ ...cfg, telegram: { ...cfg.telegram, textFormat: e.target.value } }) },
          React.createElement('option', { value: 'html' }, 'Markdown → Telegram HTML'),
          React.createElement('option', { value: 'plain' }, 'Plain text'),
        ),
        React.createElement('label', { style: { display: 'flex', gap: 8, alignItems: 'center' } },
          ...
          'Voice replies via dsh-tts (off by default)',
        ),
