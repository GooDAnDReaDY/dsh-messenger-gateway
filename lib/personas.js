...
export function createPersonaStore(filePath) {
  ...
  try {
    ...
    if (err?.code !== 'ENOENT') console.warn?.('[dsh-messenger-gateway] personas load error:', err?.message || err)
  }
  ...
  function save() {
    try {
      ...
      console.warn?.('[dsh-messenger-gateway] personas save error:', err?.message || err)
    }
  }
