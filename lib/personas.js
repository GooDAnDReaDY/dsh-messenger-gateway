...
export function createPersonaStore(filePath, { logger } = {}) {
  ...
  try {
    ...
    if (err?.code !== 'ENOENT') logger?.warn?.('[dsh-messenger-gateway] personas load error:', err?.message || err)
  }
  ...
  function save() {
    try {
      ...
      logger?.warn?.('[dsh-messenger-gateway] personas save error:', err?.message || err)
    }
  }
