...
import {
  collectAssistantParts, extractImageUrls, fetchInternalImage, readAttachmentBytes,
} from './outbound.js'
import { assistantText, splitText } from './text.js'

const PLUGIN = 'dsh-messenger-gateway'

function chatKey(platform, chatId, threadId = 0) {
  return `${platform}:${chatId}:${threadId || 0}`
}
...
export class Gateway {
  ...
  async messengerSend(target, payload) {
    ...
    await adapter.sendTo(target.chatId, payload)
  }
  ...
  async messengerAsk(target, payload, timeoutMs = 300_000) {
    ...
    const rows = (payload.buttons || []).map((row) => row.map((b) => ({
      text: b.text,
      callback_data: `${token}:${b.id}`,
    })))
    for (const row of rows) for (const btn of row) this.callbackIndex.set(btn.callback_data, token)
    const replyMarkup = rows.length ? { inline_keyboard: rows } : undefined
    await adapter.sendTo(target.chatId, { text: payload.text, replyMarkup })
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pendingAsks.delete(token)
        reject(new Error('messenger.ask timed out'))
      }, timeoutMs)
      this.pendingAsks.set(token, { resolve, reject, timer, target })
    })
  }
  ...
  async handleCallback(cb) {
    const token = this.callbackIndex.get(cb.data)
    const buttonId = token ? cb.data.slice(token.length + 1) : cb.data
    if (token && this.pendingAsks.has(token)) {
      const pending = this.pendingAsks.get(token)
      clearTimeout(pending.timer)
      this.pendingAsks.delete(token)
      await cb.answer('OK')
      try { await cb.editMessage(cb.message?.text || 'Done', undefined) } catch {}
      pending.resolve({ buttonId, data: cb.data })
      return
    }
    if (buttonId === 'model') await cb.answer('Model picker: use Web UI for now')
  ...
  async buildUserContent(input, signal) {
    ...
    for (const att of attachments) {
      if (att.kind === 'photo') {
        ...
        const ref = await this.ctx.attachments.saveImage({ data: bytes, mediaType: att.mime || 'image/jpeg', ...(att.name ? { name: att.name } : {}) })
        blocks.push({ type: 'image', attachment: ref })
      } else if (att.kind === 'voice' || att.kind === 'audio') {
        const bytes = new Uint8Array(await readFile(att.path))
        const transcript = await transcribeVoice(this.baseUrl(), bytes, att.mime || 'audio/ogg', 'message')
        parts.push(transcript ? `[Голосовое: ${transcript}]` : '[Голосовое: не удалось распознать]')
      } else {
  ...
  async buildOutboundFiles(collector, signal) {
    const files = []
    for (const ref of collector.images) {
      try { files.push(await readAttachmentBytes(this.ctx, ref)) } catch (e) {
        this.ctx.logger?.warn?.(`outbound image: ${e.message}`)
      }
    }
    const urls = extractImageUrls(collector.parts.join('\n'))
    for (const path of urls) {
      try { files.push(await fetchInternalImage(this.baseUrl(), path, signal)) } catch {}
    }
    return files
  }
  ...
  async runTurn(chat, input, signal) {
    ...
    try {
      ...
      const files = await this.buildOutboundFiles(collector, signal)
      ...
      if (this.config.tts?.enabled && answer) {
        try {
          const spoken = await speakText(this.baseUrl(), answer.slice(0, this.config.tts.maxChars || 4000))
          await reply({ files: [{ bytes: spoken.audio, mime: spoken.mime, kind: 'voice', name: 'reply.ogg' }] })
        } catch (e) { this.ctx.logger?.warn?.(`tts: ${e.message}`) }
      }
    } catch (err) {
      if (!signal?.aborted) {
