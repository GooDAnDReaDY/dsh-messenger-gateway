...
export class Gateway {
  ...
  async handleMessage(input) {
    const { platform, chatId, userId, threadId = 0, text, replyText, reply } = input
    ...
    try {
      ...
      if (incomingPhotoOnly && photoOnlyMode === 'prompt') {
        ...
        const msg = n === 1
          ? 'Фото получено. Напишите вопрос — например: «что на фото?»'
          : `Получено ${n} фото. Напишите вопрос.`
        return reply(msg)
      }
  ...
  async runTurn(chat, input, signal) {
    ...
    const collector = { parts: [], lastText: '', images: [], reason: undefined }
    ...
    try {
      ...
      const answer = stripReasoningPreamble(stripImageUrls(collector.lastText || collector.parts.join('\n\n')))
