...
export class Gateway {
  ...
  async handleCallback(cb) {
    if (cb.userId && !this.isUserAllowed(cb.userId)) {
      try { await cb.answer(t('msg.not_allowed', {}, 'en')) } catch (err) { this.recordApiFailure('cb.answer.not_allowed', err) }
      return
    }
    ...
    if (askToken && this.pendingAsks.has(askToken)) {
      ...
      if (pending.isMulti) {
        if (action.kind === 'toggle') {
          ...
          try {
            ...
            this.recordApiFailure('cb.editMessage.ask_multi_toggle', err)
          }
        ...
        if (action.kind === 'page') {
          ...
          try {
            ...
            this.recordApiFailure('cb.editMessage.ask_multi_page', err)
          }
          await cb.answer()
          return
        }
        if (action.kind === 'done') {
          ...
          releaseCallbacks(this.callbackIndex, pending.callbackKeys)
          await cb.answer('OK')
          try { await cb.editMessage(cb.message?.text || 'Done', REMOVE_KEYBOARD) } catch (err) { this.recordApiFailure('cb.editMessage.ask_multi_done', err) }
        ...
        if (action.kind === 'cancel') {
          ...
          try { await cb.editMessage(cb.message?.text || 'Cancelled', REMOVE_KEYBOARD) } catch (err) { this.recordApiFailure('cb.editMessage.ask_multi_cancel', err) }
      ...
      releaseCallbacks(this.callbackIndex, pending.callbackKeys)
      await cb.answer('OK')
      try { await cb.editMessage(cb.message?.text || 'Done', REMOVE_KEYBOARD) } catch (err) { this.recordApiFailure('cb.editMessage.ask_single_done', err) }
    ...
    if (cb.data?.startsWith('mdl:')) {
      ...
      if (sub === 'p') {
        ...
        try {
          ...
          this.recordApiFailure('cb.editMessage.model_picker_provider', err)
        }
        return
      }
      ...
      if (sub === 'pg') {
        ...
        try {
          ...
          this.recordApiFailure('cb.editMessage.model_picker_page', err)
        }
        return
      }
      // Back to providers
      if (sub === 'back') {
        ...
        try {
          ...
          this.recordApiFailure('cb.editMessage.model_picker_back', err)
        }
        return
      }
      // Select model
      if (sub === 's') {
        ...
        try {
          ...
          try {
            ...
            this.recordApiFailure('cb.editMessage.model_picker_switched', err)
          }
        } catch (err) {
  ...
  async handleCommand(key, text, input) {
    ...
    if (cmd === '/rewind') {
      ...
      try { await sessions?.flush(chat.agent.session) } catch (err) { this.logger?.debug?.('rewind session flush error:', err?.message || err) }
    ...
    if (cmd === '/fork') {
      ...
      try {
        ...
        if (newChat.agent?.session && oldMessages.length) {
          ...
          const sessions = this.ctx.get?.('sessions') || this.ctx.sessions
          try { await sessions?.flush(newChat.agent.session) } catch (err) { this.logger?.debug?.('fork session flush error:', err?.message || err) }
        }
    ...
    if (cmd === '/setalert') {
      ...
      this.config.telegram = nextTg
      try { await this.hooks?.persistHomes?.(nextTg) } catch (err) { this.logger?.warn?.('persistHomes error:', err?.message || err) }
    ...
    if (cmd === '/top') {
      ...
      try { activeReminders = (await this.scheduler.list()).length } catch (err) { this.logger?.debug?.('scheduler.list error in /top:', err?.message || err) }
      let currentModel = 'not set'
      try {
        const m = this.resolveAgentModel()
        currentModel = `${m.provider}/${m.model}`
      } catch (err) {
        this.logger?.debug?.('resolveAgentModel error in /top:', err?.message || err)
      }
      ...
      if (att.kind === 'photo' || (att.kind === 'sticker' && att.mime?.startsWith('image/'))) {
        ...
        if (att.kind === 'document' && att.path) {
          try {
            ...
            this.logger?.debug?.('parseDocument fallback:', err?.message || err)
          }
        }
  ...
  async runTurn(chat, input, signal) {
    ...
    try {
      ...
      if (signal.aborted) {
        if (progress) try { await progress.remove() } catch (err) { this.logger?.debug?.('progress.remove failed on abort:', err?.message || err) }
        if (typeof react === 'function') react('').catch?.(() => {})
        const stoppedMsg = t('msg.turn_stopped', {}, this.resolveLocale(input))
        if (stream) try { await stream.finalize(stoppedMsg) } catch (err) { this.recordApiFailure('stream.finalize.stopped', err) }
        else return reply(stoppedMsg)
        return
      }
      const sessions = this.ctx.get?.('sessions') || this.ctx.sessions
      await sessions?.flush?.(chat.agent.session)
      if (progress) try { await progress.remove() } catch (err) { this.logger?.debug?.('progress.remove failed after turn:', err?.message || err) }
      progress = null
      if (typeof react === 'function') react('').catch?.(() => {})
      ...
      if (!signal?.aborted) {
        ...
        try {
          ...
          this.recordApiFailure('reply.exception_turn', repErr)
        }
      }
    } finally {
      stopTyping()
      if (progress) try { await progress.remove() } catch (err) { this.logger?.debug?.('progress.remove failed in finally:', err?.message || err) }
      if (typeof react === 'function') react('').catch?.(() => {})
      chat.turnActive = false
      chat.abort = undefined
