...
export function createPairingStore(filePath) {
  ...
  if (filePath && existsSync(filePath)) {
    try {
      ...
      console.warn?.('[dsh-messenger-gateway] pairing store read error:', err?.message || err)
    }
  }
  const persist = () => {
    if (!filePath) return
    try {
      ...
      console.warn?.('[dsh-messenger-gateway] pairing store persist error:', err?.message || err)
    }
  }
  const prune = () => {
    const now = Date.now()
