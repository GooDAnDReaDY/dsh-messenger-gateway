  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (payload.telegram && !String(payload.telegram.webhookSecret || '').trim() && payload.telegram.webhookSecret !== undefined) {
          // empty string means keep unless explicitly clearing via special? keep current
          if (!Object.prototype.hasOwnProperty.call(payload.telegram, 'webhookSecret') || payload.telegram.webhookSecret === '') {
