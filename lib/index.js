...
function publicConfig(cfg) {
  return {
    ...
    media: { maxDocBytes: cfg.media.maxDocBytes, maxTextInjectBytes: cfg.media.maxTextInjectBytes },
    tts: cfg.tts,
    agent: cfg.agent,
  }
}
...
export function apply(ctx, config) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const gw = getGw()
      writeJson(res, 200, { ok: true, running: Boolean(gw), adapters: gw?.messenger.adapters() || [], activeChats: gw?.messenger.activeChats() || 0, config: publicConfig(source()) })
    },
  ...
  for (const [path, method] of [
    ['/dsh-messenger-gateway/messenger/send', 'send'],
    ['/dsh-messenger-gateway/messenger/progress', 'progress'],
  ]) {
    ctx.effect(() => ctx.webServer.register({
      kind: 'exact', path,
      handler: async (req, res) => {
        ...
        try { payload = JSON.parse((await readBody(req)).toString('utf8') || '{}') } catch { return writeJson(res, 400, { ok: false, error: 'invalid json' }) }
        try {
          await gw.messenger[method](payload.target, payload)
          writeJson(res, 200, { ok: true })
        } catch (err) {
          writeJson(res, 502, { ok: false, error: err.message })
        }
      },
    }), `dsh-messenger-gateway: ${method}`)
  }

  ctx.effect(() => ctx.webServer.register({
    kind: 'exact', path: '/dsh-messenger-gateway/messenger/ask',
    handler: async (req, res) => {
      if (req.method !== 'POST') return writeJson(res, 405, { ok: false, error: 'POST only' })
      const gw = getGw()
      if (!gw) return writeJson(res, 503, { ok: false, error: 'gateway not running' })
      ...
      try { payload = JSON.parse((await readBody(req)).toString('utf8') || '{}') } catch { return writeJson(res, 400, { ok: false, error: 'invalid json' }) }
      try {
        const out = await gw.messenger.ask(payload.target, payload, payload.timeoutMs)
        writeJson(res, 200, { ok: true, result: out })
      } catch (err) {
        writeJson(res, 502, { ok: false, error: err.message })
      }
    },
  }), 'dsh-messenger-gateway: ask')
