...
let catalogCache = new WeakMap()

export function clearModelCatalogCache() {
  catalogCache = new WeakMap()
}

export async function listModelCatalog(ctx, fallback = {}, options = {}) {
  ...
  if (llm && typeof llm === 'object' && !bypass) {
    const cached = catalogCache.get(llm)
    if (cached && (now - cached.timestamp < ttlMs)) {
      return {
        providers: [...cached.providers],
        modelsByProvider: new Map(cached.modelsByProvider),
      }
    }
  }
    ...
    try {
      ...
      for (const p of providers || []) {
        ...
        try {
          ...
          /* safe fallback to empty list when provider cannot list models */
          result.modelsByProvider.set(pId, [])
        }
      }
      if (llm && typeof llm === 'object' && result.providers.length > 0) {
        catalogCache.set(llm, {
          timestamp: now,
          providers: result.providers,
          modelsByProvider: new Map(result.modelsByProvider),
        })
      }
    } catch (err) {
      /* safe fallback when ctx.llm.listProviders is unavailable or throws */
    }
  }
